...
function setup() {
  ...
  bg=createSprite(windowWidth/2,windowHeight/2,windowWidth*5,height);
  bg.addImage("bg",bgImg);
bg.scale=1.25;


bird=createSprite(windowWidth-1800,windowHeight/2,50,50);
...
fill(255,255,255);
  ...
  stroke(0);
  strokeWeight(10);
text("OPEN THE CONSOLE TO KNOW THE HIGHEST SCORE", windowWidth/2-900,windowHeight/2-200)
 text("AFTER THE                    SCEOND ATTEMPT", windowWidth/2-900,windowHeight/2-100)
  ...
  text("SCORE: "+ score,windowWidth-700,80);
}
...
function spawnPipesDown() {
  //write code here to spawn the pipeUps
  if ((frameCount % 150 === 0) || (frameCount%75===0)){
    var pipeDown = createSprite(camera.x+width/2,windowHeight,40,10);
   ran=Math.round(random(1,3));
   switch(ran) {
